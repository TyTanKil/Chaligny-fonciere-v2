...
import type { DescriptionItem } from "@/data/types";
    ...
    >
      ...
      <div className="relative max-w-lg">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <h3 className="text-3xl text-lightgreen">{member.name}</h3>

            <p className="text-lg text-darkbeige font-[Montserrat] font-medium mt-1">
              {member.role}
            </p>
            ...
            <p className="mt-6 text-darkblue font-[Montserrat]">
              {member.description.map((item, index) => {
                switch (item.type) {
                  ...
                  case "highlightGreen":
                    return (
                      <span key={index} className="text-lightgreen font-bold">
                        {item.content}
                      </span>
                    );
